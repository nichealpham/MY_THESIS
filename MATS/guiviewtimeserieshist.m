...
for i=1:nfig
    ...
    for irow = 1:matrixrows
        for icol = 1:matrixcols
            j = j+1;
            h = subplot(matrixrows,matrixcols,(irow-1)*matrixcols+icol);
            if nbin==0
            ...
            if displaynorm
                ...
                leg = legend(sprintf('p=%1.4f',pval));
                set(leg,'Box','Off');
            end
...
eval(['web(''',dirnow,'\helpfiles\ViewTimeSeriesHistogram.htm'')'])
