function xpreV = lppreonedirT(xV,TreeRoot,winnowV,m,tau,nnei,q,T,mindist,tarintree)
% xpreV = lppreonedirT(xV,TreeRoot,winnowV,m,tau,nnei,q,T,mindist,tarintree)
% Help function called in localARpredir.m and localARfitdir.m
%========================================================================
%     <lppreonedirT.m>, v 1.0 2010/02/11 22:09:14  Kugiumtzis & Tsimpiris
...
if tarintree
    ...
    xpreM = NaN*ones(nnei,T);
    for inei=1:nnei
        xpreM(inei,:)=xV(neiindV(inei)+(m-1)*tau+1:neiindV(inei)+(m-1)*tau+T);
    end
    if nnei==1
        xpreV = xpreM';
    elseif q==0
        ...
        for j=1:T
            ...
            xpreV(j) = myV(j) + (tarV-mneiV) * lsbV;
        end
    end
elseif nnei==1
    [neiind,neidis,TreeRoot]=kdtreeidx([],tarV,TreeRoot);
    xpreV = xV(neiind+(m-1)*tau+1:neiind+(m-1)*tau+T);
else
    [neiM,neidisV,neiindV]=kdrangequery(TreeRoot,tarV,distnow);
