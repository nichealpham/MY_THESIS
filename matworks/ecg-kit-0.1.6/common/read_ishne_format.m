%% Reads ECG recording in ISHNE format
...
% Copyright 2008-2015
% 
function [ ECG heasig ann last_sample ] = read_ishne_format(filename, start_sample, end_sample)
...
if( fid > 0 )
    ...
    try 
        ...
        if( abs(muestras_guardadas - heasig.nsamp) > 584 )
            ...
            warning('read_ishne_format:CorruptFile', 'File corrupted. Check data/header integrity.')
        end
        ...
        if(samples2read <= 0)
            error('read_ishne_format:Impossible2read', ['Imposible leer ' num2str(samples2read) ' muestras'])
        end
