...
% of objects for wich B*WR*LABELB has a correct classname and neglects all
...
function w_out = rejectc_balanced(a,w,frac,type)
...
switch(type)
	case{'a','ambiguity'}
		w_out = w*classc*rejectm_balanced(a*w*classc,frac,rejname);
	case{'o','outlier'}
		conv = getout_conv(w);
		if conv > 1
		...
		w_out = w*rejectm_balanced(a*w,frac,rejname);
	otherwise
